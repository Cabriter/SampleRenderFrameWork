...
void Shader::Init(const char *vs, const char *fs){
    ...
    if(mProgram!=0){
        ...
        mProjectMatrixLocation = glGetUniformLocation(mProgram, "ProjectionMatrix");
...
void Shader::Bind(float *M, float *V, float *P){
    ...
    if(mModeMatrixLocation >= 0)glUniformMatrix4fv(mModeMatrixLocation, 1, GL_FALSE,M);
    if(mViewMatrixLocation >= 0)glUniformMatrix4fv(mViewMatrixLocation, 1, GL_FALSE,V);
    if(mProjectMatrixLocation >= 0)glUniformMatrix4fv(mProjectMatrixLocation, 1, GL_FALSE,P);
